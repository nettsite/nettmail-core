<?php

use Nettsite\NettMail\Core\Drivers\PostmarkDriver;
use Nettsite\NettMail\Core\Mail\EmailAddress;
use Nettsite\NettMail\Core\Mail\EmailMessage;
use Nettsite\NettMail\Core\Tests\Fakes\FakeHttpClient;
use Nyholm\Psr7\Factory\Psr17Factory;
use Nyholm\Psr7\Response;

it('sends an email via the postmark api', function () {
    $factory = new Psr17Factory();
    $httpClient = new FakeHttpClient(new Response(200, ['Content-Type' => 'application/json'], json_encode(['ErrorCode' => 0, 'MessageID' => 'pm-123'])));

    $driver = new PostmarkDriver('server-token', $httpClient, $factory, $factory);

    $result = $driver->send(new EmailMessage(
        from: new EmailAddress('sender@example.com', 'Sender'),
        to: [new EmailAddress('recipient@example.com', 'Recipient')],
        subject: 'Hello',
        html: '<p>Hi</p>',
    ));

    expect($result->success)->toBeTrue()
        ->and($result->messageId)->toBe('pm-123');

    $request = $httpClient->lastRequest;

    expect($request->getHeaderLine('X-Postmark-Server-Token'))->toBe('server-token');

    $body = json_decode((string) $request->getBody(), true);

    expect($body['From'])->toBe('Sender <sender@example.com>')
        ->and($body['To'])->toBe('Recipient <recipient@example.com>')
        ->and($body['HtmlBody'])->toBe('<p>Hi</p>');
});

it('returns a failure result on a postmark api error', function () {
    $factory = new Psr17Factory();
    $httpClient = new FakeHttpClient(new Response(422, ['Content-Type' => 'application/json'], json_encode(['ErrorCode' => 300, 'Message' => 'Invalid recipient'])));

    $driver = new PostmarkDriver('server-token', $httpClient, $factory, $factory);

    $result = $driver->send(new EmailMessage(
        from: new EmailAddress('sender@example.com'),
        to: [new EmailAddress('recipient@example.com')],
        subject: 'Hello',
    ));

    expect($result->success)->toBeFalse()
        ->and($result->error)->toBe('Invalid recipient');
});
